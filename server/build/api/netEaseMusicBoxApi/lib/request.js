...
function request(opts, callback) {
	...
	if (!url || typeof url !== 'string' || !/^https?:\/\//.test(url)) return callback(new Error('url is illegal, only supported http or https protocol!'));

	if (method !== 'GET' && method !== 'POST') return callback(new Error('only supported get or post method!'));
